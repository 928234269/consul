...
type Delegate interface {
	...
	RemoveFailedServer(*Server) error
}
...
type Promoter interface {
	...
	// CalculateFailedServerRemovals takes in the current state and structure outlining all the
	// failed/stale servers and will return those failed servers which the promoter thinks
	// should be allowed to be removed.
	CalculateFailedServerRemovals(*Config, *State, *FailedServers) *FailedServers
}
