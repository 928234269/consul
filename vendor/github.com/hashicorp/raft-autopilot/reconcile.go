...
// removeFailedNode is a small helper to log that the failed node is being removed and then
// execute the RemoveFailedServer of the delegate in a go routine. At least for Consul's usage
// the delegates implementation could block for a little while which is why this is put into a go routine.
func (a *Autopilot) removeFailedNode(srv *Server) {
	a.logger.Info("Attempting removal of failed server node", "id", srv.ID, "name", srv.Name, "address", srv.Address)
	go a.delegate.RemoveFailedServer(srv)
}
...
func (a *Autopilot) pruneDeadServers() error {
	...
	failed = a.promoter.CalculateFailedServerRemovals(a.delegate.AutopilotConfig(), a.GetState(), failed)

	// remove failed non voting servers
	for _, srv := range failed.FailedNonVoters {
		a.removeFailedNode(srv)
	}
